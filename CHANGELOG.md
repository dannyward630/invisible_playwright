...
- `network-probe` CLI command: launches patched Firefox and emits JSON status, final URL, launch metadata, JS-visible fingerprint snapshot, browser-side TLS/HTTP fingerprint response JSON, redacted response headers/timeline with sanitized request method/resource metadata, optional post-load click diagnostics, cookie metadata, and a compact shareable summary for debugging proxy and network-layer detector mismatches.
