...
- `network-probe` CLI command: launches patched Firefox and emits JSON status, final URL, browser-side TLS/HTTP fingerprint response JSON, redacted response headers/timeline, optional post-load click diagnostics, and cookie metadata for debugging proxy and network-layer detector mismatches.
